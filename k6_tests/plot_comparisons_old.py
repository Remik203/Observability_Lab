...
def dashboard(df, stacks, title, fname, rdir, test=None):
    ...
    sub_title = "All tests" if test is None else f"Test: {test}"
...
def business_impact_dashboard(rdir):
    ...
    fig.suptitle("Business Impact: K6 Client-Side Metrics",
    ...
    for i, stack in enumerate(stacks):
        ...
        stds = []
        for t in tests:
            vals = stack_df[stack_df["test_name"] == t]["p95_latency_ms"].dropna()
    ...
    for i, stack in enumerate(stacks):
        ...
        stds = []
        for t in tests:
            vals = stack_df[stack_df["test_name"] == t]["req_failed_rate"].dropna()
    ...
    # Dashboard E2: Boxplot p95 latency across all tests
    fig, ax = plt.subplots(figsize=(12, 7))
    fig.suptitle("Business Impact: p95 Latency Distribution per Stack",
    ...
    for stack in stacks:
        ...
        _, label = PALETTE.get(stack, ("gray", stack))
        box_labels.append(label)
        color, _ = PALETTE.get(stack, ("gray", stack))
